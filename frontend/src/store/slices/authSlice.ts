...
>(
  ...
  async (phone, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${API_URL}/auth/admin/send-otp`, { phone });
      return response.data;
    } catch (error) {
      const axiosError = error as AxiosError<AuthError>;
      return rejectWithValue(axiosError.response?.data || { message: 'Failed to send OTP' });
    }
  }
);
...
>(
  ...
  async ({ phone, otp }, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${API_URL}/auth/admin/verify-otp`, { phone, otp });
      ...
      const axiosError = error as AxiosError<AuthError>;
      return rejectWithValue(axiosError.response?.data || { message: 'Failed to verify OTP' });
    }
  }
);
...
>(
  ...
  async (refreshToken, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${API_URL}/auth/admin/refresh`, { refreshToken });
      ...
      const axiosError = error as AxiosError<AuthError>;
      return rejectWithValue(axiosError.response?.data || { message: 'Failed to refresh token' });
    }
  }
);
