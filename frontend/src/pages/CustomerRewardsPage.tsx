...
export const CustomerRewardsPage = () => {
  ...
  const rewards = [
    { id: 1, title: 'Complimentary Mocktail', points: 50, icon: Gift, color: 'text-brand-500 bg-brand-500/10' },
    { id: 2, title: '15% Discount Voucher', points: 150, icon: Ticket, color: 'text-emerald-500 bg-emerald-500/10' },
    { id: 3, title: 'Premium Table Seating', points: 300, icon: Medal, color: 'text-purple-500 bg-purple-500/10' },
    { id: 4, title: 'VIP Tasting Menu Access', points: 500, icon: Gem, color: 'text-blue-500 bg-blue-500/10' },
  ];
            ...
            >
               ...
               <div className="relative z-10 flex flex-col items-center">
                  ...
                  <div className="flex items-baseline gap-4 mb-4">
                     <span className="text-8xl font-black italic tracking-tighter">{customerUser?.loyaltyPoints || 0}</span>
                 ...
                 >
                    ...
                    <div className="flex items-center justify-between gap-4">
                       <div className="flex-1 h-3 bg-slate-50 dark:bg-slate-950/50 rounded-full overflow-hidden">
                          <motion.div 
                            initial={{ width: 0 }} 
                            animate={{ width: `${Math.min(100, (customerUser?.loyaltyPoints || 0) / reward.points * 100)}%` }}
                            transition={{ duration: 1, delay: 0.5 }}
                            className="h-full bg-brand-500 rounded-full"
                          />
                       </div>
                       <span className="text-[10px] font-black uppercase text-slate-400 tracking-tighter">
                         {Math.min(100, Math.round((customerUser?.loyaltyPoints || 0) / reward.points * 100))}%
                       </span>
                    </div>
                    
                    <button 
                      disabled={(customerUser?.loyaltyPoints || 0) < reward.points}
                    ...
                    >
                      {(customerUser?.loyaltyPoints || 0) >= reward.points ? (
                        <>Claim Reward <ArrowRight size={14} /></>
                      ) : (
                        <><Flame size={14} /> {(reward.points - (customerUser?.loyaltyPoints || 0))} Pts Needed</>
                      )}
                    </button>
                 </motion.div>
               ))}
