...
export const PublicLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans selection:bg-amber-500/20 selection:text-amber-600 dark:selection:text-amber-500 text-slate-800 dark:text-slate-200 transition-colors duration-300 flex flex-col relative w-full overflow-x-hidden">
            ...
            <div className="flex flex-col gap-8 text-center text-3xl font-black uppercase italic tracking-tight">
              {navLinks.map((link) => (
                 <Link 
                   key={link.path}
                   to={link.path} 
                   className={`transition-colors ${location.pathname === link.path ? 'text-amber-500' : 'hover:text-amber-500 text-slate-800 dark:text-white'}`}
      ...
      <footer className="relative z-10 bg-slate-900 dark:bg-slate-950 pt-16 pb-10 border-t border-slate-800 w-full mt-auto">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
          ...
          <div>
            ...
            <div className="flex flex-col gap-4">
              <Link to="/services" className="text-slate-400 hover:text-amber-500 font-medium transition-colors">Services</Link>
              <Link to="/about" className="text-slate-400 hover:text-amber-500 font-medium transition-colors">About Us</Link>
              <Link to="/contact" className="text-slate-400 hover:text-amber-500 font-medium transition-colors">Contact</Link>
            </div>
          </div>
          <div>
            <h4 className="text-white font-bold uppercase tracking-widest text-xs mb-6">Legal & Support</h4>
            <div className="flex flex-col gap-4">
              <Link to="/help" className="text-slate-400 hover:text-amber-500 font-medium transition-colors">Help Center</Link>
              <Link to="/privacy" className="text-slate-400 hover:text-amber-500 font-medium transition-colors">Privacy Policy</Link>
              <Link to="/terms" className="text-slate-400 hover:text-amber-500 font-medium transition-colors">Terms of Service</Link>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row items-center justify-between pt-10 border-t border-white/5 gap-6">
           ...
           <div className="flex gap-6">
             <Link to="/help" className="text-xs font-bold text-slate-500 hover:text-white transition-colors">Help</Link>
             <Link to="/privacy" className="text-xs font-bold text-slate-500 hover:text-white transition-colors">Privacy</Link>
             <Link to="/terms" className="text-xs font-bold text-slate-500 hover:text-white transition-colors">Terms</Link>
           </div>
        </div>
      </footer>
    </div>
